...
if (!isserver || !isdedicated) then {
	{
		null = [_x,_feedArray,server] execVM "Scripts\Gopro\Direct_gopro.sqf";
		_x setObjectTexture [0,'#(argb,8,8,3)color(1,1,1,0)'];
...

{
	...
	if (isplayer _x) then {_x addEventHandler ["Respawn","CALL fnc_PLAYER_KILLED"];}
	else{_x addmpEventHandler ["MPkilled","CALL fnc_AI_KILLED"];};
}foreach _feedArray;
...
[_monitorArray,_MonitorName] spawn {
	...
	while {true} do {
		...
		if (_newArray != _oldArray || (count _feedArray) <= 1) then{	
			{
				null = [_x,_feedArray] execVM "Scripts\Gopro\Direct_gopro.sqf";			
			}foreach _monitorArray;
		};
	};
};